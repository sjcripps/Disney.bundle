...
####################################################################################################
@handler(PLUGIN_PREFIX, NAME, "icon-default.png", "art-default.jpg")
def MainMenu():
  oc = ObjectContainer()
...
def Shows(title, group):
  ...
  for show in json['stack'][0]['data']:
    ...
    url = show['href']
    oc.add(DirectoryObject(key=Callback(Videos, title=title, thumb=thumb, url=url), title=title, summary=summary, thumb=Resource.ContentsOfURLWithFallback(thumb)))
  return oc
  ...
  for group in json['stack']:
    if group['type'] == 'video':
        for clip in group['data']:
            ...
            url = clip['href']
            oc.add(VideoClipObject(url=url, title=clip_title, summary=summary, duration=duration, thumb=Resource.ContentsOfURLWithFallback(clip_thumb, fallback=[thumb, 'icon-default.png'])))
  return oc
